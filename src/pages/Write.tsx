...
const INITAIL_POST_DATA = {title: "", content: "", tags: ""};
...
export default function Write() {
    ...
    useQuery({
        queryFn: () => httpRequest.get(`${url}/posts/${postId}`),
        ...
        onSuccess: (data) => {
            setPost({
                ...
                tags: Array.isArray(data.data.post.tags) 
                    ? data.data.post.tags.join(', ') 
                    : data.data.post.tags || "",
            });
        },
    });
    ...
    const {refetch: updatePost} = useQuery({
        queryFn: () => {
            const tagsArray = typeof post.tags === 'string' 
                ? post.tags.split(',').map(tag => tag.trim()).filter(tag => tag.length > 0)
                : post.tags || [];
            return httpRequest.put(`${url}/posts/${postId}`, {
                title: post.title,
                content: post.content,
                tags: tagsArray
            });
    ...
    return (
        <>
            <WriteNavbar
                ...
                disabled={!(post.title && post.title.length > 6 && post.content && post.content.length > 15)}
            />
            <div
                style={{
                    display: "flex",
